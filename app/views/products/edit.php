...
<form method="POST" action="/webbanhang/public/Product/update" enctype="multipart/form-data" style="background: #f7fafc; padding: 30px; border-radius: 15px;">
    ...
    <div class="form-group">
        ...
        <div class="text-center p-3" style="background: white; border-radius: 10px; border: 2px dashed #ddd;">
            <?php if (!empty($product->image)): ?>
                <img src="<?php echo htmlspecialchars($product->image); ?>" 
                     alt="Current Image" 
                     style="max-width: 300px; max-height: 300px; border-radius: 10px;">
                <p class="mt-2 mb-0 text-muted">
                    <i class="fas fa-check-circle text-success"></i> Đã có ảnh
                </p>
            <?php else: ?>
    ...
    <div class="form-group">
        <label for="image">
            <i class="fas fa-upload"></i> Upload hình ảnh mới
        </label>
        <div class="custom-file">
            <input type="file" 
                   ...
                   name="image" 
                   accept="image/*">
            <label class="custom-file-label" for="image">Chọn hình ảnh mới (để trống nếu không muốn đổi)...</label>
        </div>
        <small class="form-text text-muted">
            <i class="fas fa-info-circle"></i> Để trống nếu không muốn thay đổi ảnh. Định dạng: JPG, PNG, GIF. Tối đa: 2MB
        </small>
    </div>

    <!-- Preview ảnh mới -->
    <div class="form-group" id="preview-container" style="display: none;">
        <label>Preview ảnh mới:</label>
        <div class="text-center">
            <img id="image-preview" src="" alt="Preview" style="max-width: 300px; max-height: 300px; border-radius: 10px; border: 2px solid #667eea;">
        </div>
    </div>
